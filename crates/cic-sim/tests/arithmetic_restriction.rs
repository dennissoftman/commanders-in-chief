...
//!
//! This is *the* simulation crate the decision was written about: everything here is either
//! simulation state or the machinery that advances it, so a platform transcendental anywhere in
//! `src/` is a desync waiting for the first pair of machines with different C libraries. The
//! kernel's own arithmetic is almost entirely integer; the scan is what keeps that true as
//! subsystems accumulate above it.
...
fn offences(source: &str) -> Vec<(usize, String)> {
    ...
    for (index, full_line) in shipped.lines().enumerate() {
        // A comment is not a call: documentation may name the forbidden functions while explaining
        // this very rule.
        ...
        for name in FORBIDDEN {
            ...
            // immediately followed by its argument list. Without the trailing parenthesis this
            // would fire on `.expect(`, and without the leading separator it would fire on
            // `sin_turns`.
            for prefix in [".", "::"] {
...
fn no_shipped_code_calls_a_platform_transcendental() {
    ...
    assert!(scanned >= 8, "only {scanned} source files were scanned");
    assert!(
        failures.is_empty(),
...
fn the_scanner_does_not_fire_on_what_is_permitted() {
    // The permitted set, and the near-misses that a looser pattern would flag.
    for permitted in [
        "//! The kernel never calls `sin()`; `cic_math::sin_turns` is prose here.",
        "let root = value.sqrt();",
...
fn a_test_module_may_use_an_oracle() {
    // The scan stops at the first test module, so tests can measure against a platform function
    // where that is the honest comparison.
    let source = "fn ours(x: f64) -> f64 { x }\n\
                  #[cfg(test)]\n\
