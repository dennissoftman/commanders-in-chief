...
fn map_view_output_is_reproducible_and_follows_the_explicit_pose() {
    ...
    // A different pose through the same path must produce a different image, and PPM is selected
    // from the output extension rather than a separate command.
    let rotated_path = root.join("terrain-rotated.ppm");
    ...
    assert!(
        fs::read(&rotated_path)
            .expect("read PPM capture")
            .starts_with(b"P6\n96 64\n255\n")
    );
    ...
    assert!(
        String::from_utf8_lossy(&refused.stderr).contains("must end in .png or .ppm"),
        "{}",
